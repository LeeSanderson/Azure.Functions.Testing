...
using Azure.Functions.Testing.Cli.Actions.HostActions.WebHost;
...
namespace Azure.Functions.Testing.Cli.Actions.HostActions
{
    internal class StartHostAction : BaseAction
    {
        ...
        public string LanguageWorkerSetting { get; set; } = string.Empty;
        ...
        public override ICommandLineParserResult ParseArgs(string[] args)
        {
            ...
            Parser
                ...
                .SetDefault(hostSettings.LocalHttpPort == default ? DefaultPort : hostSettings.LocalHttpPort)
                .Callback(p => Port = p);
            ...
            if (VerboseLogging != null && !VerboseLogging.Value && verboseLoggingArgExists)
            {
                VerboseLogging = null;
            }
            return parserResult;
        ...
        private async Task<IWebHostBuilder> BuildWebHostBuilder(ScriptApplicationHostOptions hostOptions, Uri listenAddress, Uri baseAddress, X509Certificate2? certificate)
        {
            ...
            var settings = await GetConfigurationSettings(hostOptions.ScriptPath, baseAddress);
            ...
            if (UseHttps)
            {
                if (certificate == null)
                {
                    throw new CliException("UseHttp true but no certificate available");
                }

                defaultBuilder
                .UseKestrel(options =>
                {
                    options.Listen(IPAddress.Any, listenAddress.Port, listenOptions =>
                    {
            ...
            return defaultBuilder
                ...
                .ConfigureServices((context, services) =>
                {
                    ...
                    {
                        services.AddSingleton<IConfigureBuilder<IServiceCollection>>(_ =>
                            new DotNetIsolatedDebugConfigureBuilder());
                    }
                });
        }

        private async Task<IWebHost> BuildWebHost(ScriptApplicationHostOptions hostOptions, Uri listenAddress,
            Uri baseAddress, X509Certificate2? certificate)
        {
            return (await BuildWebHostBuilder(hostOptions, listenAddress, baseAddress, certificate)).Build();
        }
        ...
        private async Task<IDictionary<string, string?>> GetConfigurationSettings(string scriptPath, Uri uri)
        {
            ...
            settings.AddRange(connectionStrings.ToDictionary<ConnectionString, string, string?>(
            ...
            await CheckNonOptionalSettings(
                settings.Union(environment).Union(userSecrets).ToDictionary(e => e.Key, e => e.Value), 
                scriptPath, 
                userSecretsEnabled);
        ...
        private async Task<(Uri listenUri, Uri baseUri, X509Certificate2? cert, ScriptApplicationHostOptions hostOptions)> PrepareAndSetup()
        {
            ...
            // Suppress AspNetCoreSuppressStatusMessages
            EnvironmentHelper.SetEnvironmentVariableAsBoolIfNotExists(Constants.AspNetCoreSuppressStatusMessages);
            ...
            var (listenUri, baseUri, certificate) = await Setup();

            await ExtensionBundleHelper.GetExtensionBundle();

            return (listenUri, baseUri, certificate, hostOptions);
        }

        public override async Task RunAsync()
        {
            var (listenUri, baseUri, certificate, hostOptions) = await PrepareAndSetup();
        ...
        public async Task<IWebHostBuilder> BuildWebHostBuilder()
        {
            var (listenUri, baseUri, certificate, hostOptions) = await PrepareAndSetup();

            return await BuildWebHostBuilder(hostOptions, listenUri, baseUri, certificate);
        }
        ...
        private void ValidateAndBuildHostJsonConfigurationIfFileExists(ScriptApplicationHostOptions hostOptions)
        {
            ...
            //BuildHostJsonConfiguration only if host.json file exists.
        ...
        private bool IsPreCompiledFunctionApp()
        {
            ...
            foreach (var directory in FileSystemHelpers.GetDirectories(Environment.CurrentDirectory))
            {
                ...
                if (File.Exists(functionMetadataFile))
                {
                    ...
                    var extension = Path.GetExtension(functionMetadata?.ScriptFile)?.ToLowerInvariant().TrimStart('.');
        ...
        internal static async Task CheckNonOptionalSettings(IDictionary<string, string?> secrets, string scriptPath, bool skipAzureWebJobsStorageCheck = false)
        {
            ...
            try
            {
                ...
                foreach (var (filePath, functionJson) in functionsJsonObjects)
                {
                    foreach (var bindingToken in functionJson["bindings"]!)
                    {
                        if (bindingToken is JObject binding)
                        {
                            foreach (var token in binding)
                            {
                                if (token.Key is "connection" or "apiKey" or "accountSid" or "authToken")
                                {
                                    var appSettingName = token.Value?.ToString();
                                    if (string.IsNullOrWhiteSpace(appSettingName))
                                    {
                                        ColoredConsole.WriteLine(
                                            WarningColor($"Warning: '{token.Key}' property in '{filePath}' is empty."));
                                    }
                                    else if (token.Key == "connection" && !ConnectionExists(secrets, appSettingName) ||
                                             token.Key != "connection" && !secrets.Any(v =>
                                                 v.Key.Equals(appSettingName, StringComparison.OrdinalIgnoreCase)))
                                    {
                                        ColoredConsole
                                            .WriteLine(WarningColor(
                                                $"Warning: Cannot find value named '{appSettingName}' in {SecretsManager.AppSettingsFileName} that matches '{token.Key}' property set on '{binding["type"]}' in '{filePath}'. " +
