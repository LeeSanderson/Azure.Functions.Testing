...
namespace Azure.Functions.Testing.Cli.Helpers
{
    ...
    {
        private const string PythonDefaultExecutableVar = "languageWorkers:python:defaultExecutablePath";
        ...
        private static WorkerLanguageVersionInfo? _pythonVersionCache;
        ...
        {
            ColoredConsole.WriteLine(AdditionalInfoColor("The new Python programming model is in public preview. Learn more at "));
            ...
            if (InVirtualEnvironment)
            {
                try
                {
                    ...
                    if (FileSystemHelpers.DirectoryExists(Path.Join(Environment.CurrentDirectory, virtualEnvName)))
                    {
                        ...
                        await using var fileStream = FileSystemHelpers.OpenFile(funcIgnorePath, FileMode.Append, FileAccess.Write);
        ...
        public static async Task<WorkerLanguageVersionInfo?> GetEnvironmentPythonVersion()
        {
            ...
            // This also allows for an escape path for complicated environments.
            var pythonDefaultExecutablePath = Environment.GetEnvironmentVariable(PythonDefaultExecutableVar);
            ...
            var recommendedPythonWorker = versions.FirstOrDefault(IsVersionSupported);
            ...
            var python3Worker = versions.FirstOrDefault(w => w?.Major == 3);
            ...
            var anyPythonWorker = versions.FirstOrDefault(w => !string.IsNullOrEmpty(w?.Version));
        ...
        {
            if (overwrite || string.IsNullOrEmpty(Environment.GetEnvironmentVariable(PythonDefaultExecutableVar)))
            {
                Environment.SetEnvironmentVariable(PythonDefaultExecutableVar, pyExe, EnvironmentVariableTarget.Process);
            }
            if (StaticSettings.IsDebug)
            {
                ColoredConsole.WriteLine(VerboseColor($"{PythonDefaultExecutableVar} is set to {Environment.GetEnvironmentVariable(PythonDefaultExecutableVar)}"));
            }
        }

        private static async Task<WorkerLanguageVersionInfo?> GetVersion(string pythonExe)
        {
            var pythonExeVersionTask = VerifyVersion(pythonExe);
            string pythonExeVersion = await Utilities.SafeExecution(async () => await pythonExeVersionTask);
