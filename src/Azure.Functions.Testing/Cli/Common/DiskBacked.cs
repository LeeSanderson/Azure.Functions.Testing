...
namespace Azure.Functions.Testing.Cli.Common
{
    ...
    internal class DiskBacked<T> where T : new()
    {
        ...
        public static DiskBacked<T> Create(string path)
        {
            ...
            {
                T value = JsonConvert.DeserializeObject<T>(FileSystemHelpers.ReadAllTextFromFile(path)) ?? new T();
        ...
        {
            encryptionReason ??= DefaultEncryptionReason;
            if (FileSystemHelpers.FileExists(path))
            {
                var bytes = FileSystemHelpers.ReadAllBytes(path);
                var content = Decrypt(bytes, encryptionReason);
                T value = JsonConvert.DeserializeObject<T>(content) ?? new T();
