...
namespace Azure.Functions.Testing.Cli.Common
{
    ...
    {
        private static readonly IFileSystem Default = new FileSystem();
        ...
        {
            await using var fileStream = OpenFile(path, FileMode.Open, FileAccess.Read, FileShare.ReadWrite | FileShare.Delete);
        ...
        {
            await using var fileStream = OpenFile(path, FileMode.Create, FileAccess.Write, FileShare.ReadWrite | FileShare.Delete);
        ...
        {
            await using var fileStream = OpenFile(path, FileMode.OpenOrCreate, FileAccess.Write);
        ...
        {
            var ignoredDirectories = new List<string> { ".git", ".vscode" };
            ...
            foreach (var file in GetFiles(path, ignoredDirectories.ToArray(), ignoredFiles))
            {
