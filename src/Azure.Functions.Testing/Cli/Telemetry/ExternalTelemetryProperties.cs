    ...
    internal static class ExternalTelemetryProperties
    {
        ...
        internal static string GetInstallationType()
        {
            ...
            try
            {
                ...
                return ((string?)Registry.GetValue(key, valueName, defaultValue: "")) ?? string.Empty;
        ...
        internal static string GetLibcRelease()
        {
            ...
            try
            {
                return Marshal.PtrToStringUTF8(gnu_get_libc_release()) ?? string.Empty;
            }
            // Catch everything: this is for telemetry only
            catch (Exception e)
            {
                Debug.Assert(e is DllNotFoundException or EntryPointNotFoundException);
                return string.Empty;
            }
        }
        ...
        internal static string GetLibcVersion()
        {
            ...
            try
            {
                return Marshal.PtrToStringUTF8(gnu_get_libc_version()) ?? string.Empty;
            }
            // Catch everything: this is for telemetry only
            catch (Exception e)
            {
